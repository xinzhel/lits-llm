...
Integration with ToolUsePolicy:
    ...
    for step in steps:
        ...
        if validation and not validation['is_valid']:
            print(f"Invalid SQL detected: {validation['reasoning']}")
...
Custom Validation Prompt:
    ...
    Return JSON with: is_valid, score, reasoning, issue.'''
...
class SQLValidator:
    ...
    Args:
        ...
        require_reasoning: Whether to require the LLM to provide reasoning (default: True)
...
Your job is to rigorously evaluate SQL queries for correctness, spatial semantics, and CRS logic.

You must return exactly ONE critical issue in the "issue" field.
The value of the "issue" field MUST satisfy all requirements:
...
2. **Abstract, general, and principle-level**
   - The issue must be a general description of the type of error.
   - The issue MUST NOT mention specific table names, column names, literals, or SQL details.

3. **Single critical issue only**
   - Even if multiple issues exist, summarize only the most critical problem 
     as a general principle.

4. **Allowed Format**
   - The issue must be a short, general statement describing a single class of error.
   - Examples of correct forms:
       - "Using geometry in EPSG:4326 with meter-level distance thresholds causes incorrect results because distances are interpreted in degrees."
       - "Using geography is required when distance measurements need to be interpreted in meters."
       - "Using a projected CRS is necessary for accurate meter-based distance comparisons."

Your evaluation must still check:
- Syntax correctness (PostgreSQL/PostGIS)
- Semantic validity (tables, columns, operations)
- Spatial correctness, including CRS unit rules
...
Return a JSON object:
{
    ...
    "issue": "Self-contained general description of the single critical problem"
}
"""
    def __init__(
        self,
        ...
        Args:
            ...
            require_reasoning: Whether to require reasoning in validation
        """
        ...
        self.validation_prompt = validation_prompt or self.DEFAULT_VALIDATION_PROMPT
        ...
        # Set system prompt
        self.base_model.sys_prompt = self.validation_prompt
    ...
    def evaluate(
        ...
        """Evaluate a step and return a single issue string for policy feedback.
        ...
        Returns:
            Single string describing the issue, or None if valid
        ...
        Example:
            ...
            ```
        """
        result = self.validate(step, context, user_intent, query_idx, 
    ...
    def validate(
        ...
        if result and result.get('issue') and policy_model_name and task_type:
    ...
    def _save_eval(
        ...
        """Save validation issue to jsonl file.
        
        Args:
            result: Validation result with issue
        ...
        logger.debug(f"Saved issue to {saver.filepath}")
    ...
    def load_eval_as_prompt(
        ...
        """Load saved issues and format as prompt component for policy.
        
        Args:
            policy_model_name: Policy model name
            task_type: Task type
            max_issues: Maximum number of recent issues to include
        
        Returns:
            Formatted prompt string with issues, or empty string if no issues
        
        Example:
            ```python
            # Load issues and inject into policy prompt
        ...
        try:
            ...
            # Get recent issues (last max_issues)
            recent_issues = saver.results[-max_issues:]
            
            if not recent_issues:
                return ""
            
            # Format as prompt
            prompt_parts = ["**Previous SQL Errors to Avoid:**\n"]
            
            for idx, record in enumerate(recent_issues, 1):
